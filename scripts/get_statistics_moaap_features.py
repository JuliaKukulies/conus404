...
weather_type = str(sys.argv[3])

############# read in the data ####################################
savedir = Path('/glade/work/kukulies/pe_conus404/tests/')
...
for idx, tt in enumerate(conus.time.values): 
    ...
    atmospheric_feature_4km_t = regrid_data(era_var, conus)
...
feature_df = get_feature_dataframe(feature_dict)

# compute the bulk statistics for one month
print('compute bulk statistics...')
print(datetime.datetime.now(), flush = True)
stats_df = get_statistics(feature_df, segments, conus)

# save output
fname = savedir / str('features_' + weather_type+ '_'+ year + '_' + month + '_'+ '.nc') 
xr.DataArray(stats_df).to_netcdf(fname) 
